...
class LazyLoadingTablePage()(implicit val xsh: XSPageHandle) extends MenuPage with DemoCodePage {

  override def pageCodeFile = "app/pages/tableexamples/LazyLoadingTablePage.scala"
  override def pageTitle: String = Menu.tableSectionLazyLoadingTable.name
  ...
  override def render: (NodeSeq) => NodeSeq = super.render andThen (
    ".page-contents" #> {
      TH.Row("margin-bottom: 30px;")(
        TH.Col().md12 {
          // LazyLoadingTablePage
          new TH.Widget {
            override def style: TH.WidgetStyle.Value = TH.WidgetStyle.Primary
            override def solid: Boolean = true
            override def widgetTitle: String = "Lazy Loading Table Example"
            override def widgetBody: NodeSeq = new TH.LazyLoadingTable {
            ...
            // LazyLoadingTablePage
            codeWidget("LazyLoadingTablePage")
        }
      )
    }
    )
