...
class PaginationTablePage()(implicit val xsh: XSPageHandle) extends MenuPage with DemoCodePage {

  override def pageCodeFile = "app/pages/tableexamples/PaginationTablePage.scala"
  override def pageTitle: String = Menu.tableSectionPaginationtables.name
      ...
      TH.Row("margin-bottom: 30px;")(
        TH.Col().md12 {
          ...
          new TH.Widget {
            ...
            override def widgetTitle: String = "Pagination Table Example"
            override def widgetBody: NodeSeq = new TH.PagTable {
              override def pagBtnsCurrentClass: String = "active"
              override def pagBtnsDisabledClass: String = "disabled"

              override def pagRows(pagOffset: Int, pagSize: Int): (Seq[R], Long) = {
                val all: Seq[R] = rows()
                val _rows = all.drop(pagOffset).take(pagSize)
                (_rows, all.size)
              }
