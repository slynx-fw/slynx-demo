...
class FixedHeaderTablePage()(implicit val xsh: XSPageHandle) extends MenuPage with DemoCodePage {

  override def pageCodeFile = "app/pages/tableexamples/FixedHeaderTablePage.scala"
  override def pageTitle: String = Menu.tableSectionFixedHeaderTable.name
  ...
  override def render: (NodeSeq) => NodeSeq = super.render andThen (
    ".page-contents" #> {
      TH.Row("margin-bottom: 30px;")(
        TH.Col().md12 {
          // FixedHeaderTablePage
          new TH.Widget {
            override def style: TH.WidgetStyle.Value = TH.WidgetStyle.Primary
            override def solid: Boolean = true
            override def widgetTitle: String = "Fixed Header Table Example"
            override def widgetBody: NodeSeq = new TH.FixedHeaderTable {
              ...
              override def rows(): Seq[ImdbList] = Profile.%?("getTop250")(new ImdbApi().getTop250.toVector)
            }.renderedTable
          }.renderedWidget ++
            // FixedHeaderTablePage
            codeWidget("FixedHeaderTablePage")
        }
      )
    }
    )
